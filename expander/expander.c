...
/*   Updated: 2024/07/27 14:04:12 by abadouab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_expand *splite_use_qoutes(t_minishell *ms, char *arg)
{
	...
	while (arg[index])
	{
		...
		while ((qoutes == 1 && arg[index] && arg[index] != '\'') || (qoutes == 2 && arg[index] && arg[index] != '\"') || (!qoutes && arg[index] && arg[index] != '\'' && arg[index] != '\"'))
			index++;
		if ((qoutes == 1 && arg[index] == '\'') || (qoutes == 2 && arg[index] == '\"'))
			index++;
...
char *splite_mult_args(t_minishell *ms, char *arg, bool status, bool option)
{
	...
	while (expand)
	{
		(!option && (*expand->value == '\'' || *expand->value == '\"')) && (option = TRUE);
		(expand->next && (*expand->next->value == '\'' || *expand->next->value == '\"')) && (op = TRUE);
		...
		((ft_strchr(expand->value, '\'') || ft_strchr(expand->value, '\"')) && (ft_strchr(expand->value, '*'))) && (ms->wildcards = FALSE);
...
void expanding(t_minishell *ms, t_tree *tree, int index)
{
	...
	while (tree->args[++index])
	{
		...
		tree->args[index] = splite_mult_args(ms, tree->args[index], TRUE,
											 FALSE);
		join_doubles(ms, tree, ft_split(&ms->leaks, tree->args[index], EOT), &index);
